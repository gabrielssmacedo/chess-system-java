...
public class ChessMatch {
	...
	public ChessPiece[][] getPieces() {
		...
		for(int i = 0; i < board.getRows(); i++) {
			for(int j = 0; j < board.getRows(); j++) {
				mat[i][j] = (ChessPiece) board.findPiece(i, j);
			}
		}
	...
	public boolean[][] possibleMoves(ChessPosition sourcePosition) {
		...
		return board.findPiece(position).possibleMoves();
	}
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		ChessPiece movedPiece = (ChessPiece)board.findPiece(target);
		
		//promotion 
		promoted = null;
		if(movedPiece instanceof Pawn) {
			if((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) {
				promoted = (ChessPiece)board.findPiece(target);
	...
	private void validateSourcePosition(Position position) {
		...
		if(currentPlayer != ((ChessPiece)board.findPiece(position)).getColor()) {
			throw new ChessException("The chosen piece is not yours");
		}
		if(!board.findPiece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is no possible moves for the chosen piece");
		}
	}
	
	private void validateTargetPosition(Position source, Position target) {
		if(!board.findPiece(source).possibleMove(target)) {
